...
static esp_ble_mesh_dev_role_t my_role = 0; // 0 or PROVISIONER
static esp_ble_mesh_client_t onoff_client;
static esp_ble_mesh_client_t user_prop_client; 
...
static esp_ble_mesh_model_pub_t user_prop_cli_pub = {
...
static esp_ble_mesh_model_t root_models[] = {
    ...
    ESP_BLE_MESH_MODEL_GEN_PROPERTY_CLI(&user_prop_cli_pub, &user_prop_client), 
...
void app_pub_msg_test(void)
{
    ...
    esp_ble_mesh_model_t my_model = root_models[1]; // ESP_BLE_MESH_MODEL_GEN_ONOFF_CLI(&onoff_cli_pub, &onoff_client);
    ...
    */
    ...
    error = esp_ble_mesh_model_publish(&my_model, ESP_BLE_MESH_MODEL_OP_GEN_USER_PROPERTY_GET, 8, &my_data, my_role);
    if (error) {
    ...
    {
        ESP_LOGE(TAG, " message publication successful"); // debugging
    }
}
 ...
 *                this API should be called in the ESP_BLE_MESH_MODEL_PUBLISH_UPDATE_EVT
 *                with the message updated.
 *
 *
 * @param[in]    model: Mesh (client) Model that's publishing the message.
 ...
 * @param[in]    device_role: Role of the device (node/provisioner) that's publishing the message of type esp_ble_mesh_dev_role_t.
 *
