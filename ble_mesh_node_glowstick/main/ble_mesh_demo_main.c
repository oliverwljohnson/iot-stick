...
static esp_ble_mesh_model_pub_t user_property_pub = { 
    ...
    .dev_role = 0, // Role of the device that is going to publish messages
};
...
static esp_ble_mesh_model_op_t user_property_op[] = {
    { ESP_BLE_MESH_MODEL_OP_GEN_USER_PROPERTY_GET, 16, 0}, // User Property Get
    { ESP_BLE_MESH_MODEL_OP_GEN_USER_PROPERTY_SET, 16, 0}, // User Property Set
    { ESP_BLE_MESH_MODEL_OP_GEN_USER_PROPERTY_SET_UNACK, 16, 0}, // User Property Set Unacknowledged
...
static esp_ble_mesh_model_t root_models[] = {
    ...
    &user_property_pub, &test_data), // SIG Model 
    ESP_BLE_MESH_MODEL_GEN_PROPERTY_CLI(&user_property_pub, &genre_client) // Music genre client model
};
...
void app_msg_send_test()
{
    /*
    enum genre {EDM, classical, rock}; // Defines the music genre selected by the user/glowstick
    genre my_selection = EDM;

    esp_ble_mesh_model_t my_model;
    esp_ble_mesh_msg_ctx_t my_ctx;
    uint32_t my_opcode;
    uint16_t my_length;
    uint8_t my_data;
    esp_err_t error;
    struct my_network_params; // network parameters
    
    my_model = root_models[1]; // 2nd element on root_models array is our custom model
    my_network_params = get_network_params();
    
    // Set the context variables
    my_ctx.net_idx = my_network_params.net_idx; // Network Index
    my_ctx.app_idx =  // App Index
    my_ctx.addr = 0xFFFF // Remote address. 0xFFFF = All nodes (Virtual addr.)
     Force sending reliably by using segment acknowledgement 
    my_ctx.send_rel = 1; 
    my_ctx.send_ttl; // TTL. Use BT_MESH_TTL_DEFAULT for default TTL.
     


    // Send the message
    error = esp_ble_mesh_server_model_send_msg();
    if (error) {
        ESP_LOGE(TAG, "%s send message failed.." __func__); // debugging
    }
    */
