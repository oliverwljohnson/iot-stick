...
#define MSG_SEND_TTL            3 // Time to Live (hop-counter)
        ...
        esp_ble_mesh_model_t *model, uint32_t opcode)
{
    ...
    common->msg_role = MSG_ROLE;//MSG_ROLE;
        ...
        esp_ble_mesh_cfg_client_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_CFG_CLIENT_SET_STATE_EVT:
        ...
        case ESP_BLE_MESH_MODEL_OP_MODEL_APP_BIND: {
            ...
            esp_ble_mesh_set_msg_common(&common, node, config_client.model, ESP_BLE_MESH_MODEL_OP_GEN_USER_PROPERTY_GET);
        ...
        esp_ble_mesh_cfg_client_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_CFG_CLIENT_GET_STATE_EVT:
        ...
        case ESP_BLE_MESH_MODEL_OP_COMPOSITION_DATA_GET: {
            ESP_LOGI(TAG, "composition data %s", bt_hex(param->status_cb.comp_data_status.composition_data->data,
                     param->status_cb.comp_data_status.composition_data->len));
            esp_ble_mesh_cfg_client_set_state_t set_state = {0};
            esp_ble_mesh_set_msg_common(&common, node, &root_models[2], ESP_BLE_MESH_MODEL_OP_APP_KEY_ADD);
            set_state.app_key_add.net_idx = prov_key.net_idx;
            set_state.app_key_add.app_idx = prov_key.app_idx;
            memcpy(set_state.app_key_add.app_key, prov_key.app_key, 16);
            err = esp_ble_mesh_config_client_set_state(&common, &set_state);
    ...
    case ESP_BLE_MESH_CFG_CLIENT_SET_STATE_EVT:
        ...
        case ESP_BLE_MESH_MODEL_OP_MODEL_APP_BIND: {
            esp_ble_mesh_generic_client_get_state_t get_state = {0};
            esp_ble_mesh_set_msg_common(&common, node, &root_models[2], ESP_BLE_MESH_MODEL_OP_GEN_USER_PROPERTY_GET);
            err = esp_ble_mesh_generic_client_get_state(&common, &get_state);
            if (err) {
...
{
    ESP_LOGI(TAG, "Generic client cb fired..");
    esp_ble_mesh_client_common_param_t common = {0};
    esp_ble_mesh_node_info_t *node = NULL;
    uint32_t opcode;
    uint16_t addr;
    ...
    ESP_LOGE(TAG, "%s: Generic Client Get State Event fired..", __func__);
        switch (opcode) {
            case ESP_BLE_MESH_MODEL_OP_GEN_USER_PROPERTY_GET: { // User prop get
                ESP_LOGE(TAG, "%s: Generic User Property Get fired..", __func__);
                esp_ble_mesh_generic_client_get_state_t get_state = {0};
                esp_ble_mesh_set_msg_common(&common, node, &root_models[2], ESP_BLE_MESH_MODEL_OP_GEN_USER_PROPERTY_GET);
                err = esp_ble_mesh_generic_client_get_state(&common, &get_state);
                if (err) {
                    ESP_LOGE(TAG, "%s: Generic User Property Get failed", __func__);
                    return;
                }
                break;
            }
            default:
            ESP_LOGI(TAG, "Generic client get took default path..");
                break;
        }
        break;
    ...
    case ESP_BLE_MESH_GENERIC_CLIENT_PUBLISH_EVT:
        ESP_LOGI(TAG, "ESP_BLE_GENERIC_PUBLISH_EVT fired...");
        break;
    case ESP_BLE_MESH_GENERIC_CLIENT_TIMEOUT_EVT:
        ESP_LOGI(TAG, "ESP_BLE_GENERIC_CLIENT_TIMEOUT_EVT fired...");
